...
public class ImageController {
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        if(checkIfOwner(imageId, session)){
            model.addAttribute("image", image);
    ...
    //Below method takes the id of the image and the current HttpSession
    //It then gets the image object and checks from the session attributes
    //if the owner of the image is the same as the owner submitting the edit request
    //if yes, it returns true else it returns false
    private Boolean checkIfOwner(Integer imageId, HttpSession session) {
        User currentUser = (User) session.getAttribute("loggeduser");
        Integer imageOwnerId = imageService.getImage(imageId).getUser().getId();
