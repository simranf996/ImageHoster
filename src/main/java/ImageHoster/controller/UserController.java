...
public class UserController {
    ...
    //This method checks whether the password supplied by the user
    //1. Contains 1 alphabet
    //2. Contains 1 number
    //3. Contains 1 special character
    private Boolean checkPasswordStrength(String userPassword){
        if(userPassword.length()<3)
            return false;
        String passwordRegex= "(?=.*[a-z])(?=.*[0-9])(?=.*[^a-z0-9])";
        Pattern strongPasswordPattern = Pattern.compile(passwordRegex, Pattern.CASE_INSENSITIVE);
        Matcher passwordMatcher = strongPasswordPattern.matcher(userPassword);
        if(passwordMatcher.find()) return true;
        else return false;
    }
}
